...
- **Status Detection**:由 Root Helper 的 status 命令输出:`down`(无 openconnect 进程且无过期网关路由)/ `route-stale`(进程已退出但当前 Profile 的网关仍有可恢复静态主机路由)/ `route-check-failed`(存在无法安全判断的路由状态)/ `connecting`(进程在但未拿到有效 IP)/ `connected <ip>`(进程在且拿到有效 IP)。有效内网 IP 仅取自 openconnect 自身输出(`Configured as <ip>`),不再以 ifconfig 猜网段。另有 `config-error:缺 <fields>`:Profile 必需字段缺失时的前置校验输出,先于任何子命令的派发,故所有命令都可能以它开头;App 将其归约为未配置。
- **Root Helper 协议词汇表**:App 侧对 Root Helper 全部输出 token 的解析只允许存在一份实现(App 的协议 adapter)。词汇即 CONTEXT.md 所列 token;新增输出必须同步改这一份 adapter 与 Root Helper 的契约测试。
- **Tunnel 状态**:Repairing / Disconnected / Connecting / Disconnecting / Connected / Reconnecting / Error。清理完成前保持 Repairing 或 Disconnecting;网络变化经防抖确认后拆隧道进入 Reconnecting(自动重连,连续失败达上限进入 Error·重连失败);路由检查/清理失败、连接超时、掉线、认证或证书失败均进入可见 Error。Reconnecting 只续接本会话内用户发起过的连接,启动残留不自动连。
- **Network Fingerprint**:Root Helper `network` 命令输出的物理网络标识(接口/IP/路由器)。连接时记为基线,此后周期比对;瞬断视为过渡态防抖,稳定变化判定网络变化。
